    ...
    def get_val(self,batch_size,mode='val',maxlen=50): # maxlen indicate that number of video frames
        ...
        for samp in caps:
            ...
            if name not in caps_ref_dict.keys():
                ...
                caps_ref_dot[name] = [cap_dot]
                caps_ref_sdot[name] = [cap_sdot]
            else:
                caps_ref_dict[name].append(cap)
                caps_ref_dot[name].append(cap_dot)
                caps_ref_sdot[name].append(cap_sdot)
        ...
        for name_f in name_set:
            ...
            if len_feat <= maxlen:
                ...
                for feat_idx in range(maxlen):
                    ...
                    mask[:,i] = 1 
            i += 1
        return features,mask,caps_ref,caps_ref_dot,caps_ref_sdot
